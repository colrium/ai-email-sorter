...
Please fully deploy the app Fly.io.

RECOMMENDED APPROACH:
Next.js(full-stack approach(TypeScript)) App Router + API Routes + Background Jobs


### **Recommended Tech Stack:**
Frontend:     Next.js 15+ (App Router) + React + Material UI
Backend:      Next.js API Routes (no separate backend)
Database:     PostgreSQL + Prisma ORM
Jobs:         BullMQ + Redis
AI:           Anthropic Claude API (Sonnet 4.5 + Opus 4)
Browser:      Puppeteer (for unsubscribe agent)
Auth:         NextAuth.js
Deployment:   Fly.io (recommended) or Render
Testing:      Jest + Playwright + React Testing Library
Monitoring:   Sentry + Fly.io logs
